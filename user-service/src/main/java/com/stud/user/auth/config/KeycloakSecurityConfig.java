...
public class KeycloakSecurityConfig {
    ...
    private static final Set<RoleName> APP_ROLES = EnumSet.allOf(RoleName.class);
    ...
    private Collection<GrantedAuthority> extractRealmAuthorities(Jwt jwt) {
        ...
        return authorities.isEmpty()
                ? List.of(new SimpleGrantedAuthority(RoleName.CLIENT.name()))
                : authorities;
    }
    ...
    private Stream<GrantedAuthority> toAuthority(Object role) {
        ...
        try {
            RoleName appRole = RoleName.valueOf(roleName);

            if (!APP_ROLES.contains(appRole)) {
                return Stream.empty();
            }

            return Stream.of(new SimpleGrantedAuthority(appRole.name()));
