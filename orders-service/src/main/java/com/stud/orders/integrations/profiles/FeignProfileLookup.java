...
public class FeignProfileLookup implements ProfileLookup {
    ...
    public ClientProfileRef getClientProfile(UUID clientProfileId) {
        return get(() -> client.getClientProfile(clientProfileId), "Client profile not found: " + clientProfileId);
    }

    @Override
    public HunterProfileRef getHunterProfile(UUID hunterProfileId) {
        return get(() -> client.getHunterProfile(hunterProfileId), "Hunter profile not found: " + hunterProfileId);
    }

    @Override
    public ClientProfileRef getClientProfileByUserId(UUID userId) {
        return get(() -> client.getClientProfileByUserId(userId), "Client profile not found for user: " + userId);
    }

    @Override
    public HunterProfileRef getHunterProfileByUserId(UUID userId) {
        return get(() -> client.getHunterProfileByUserId(userId), "Hunter profile not found for user: " + userId);
    }
